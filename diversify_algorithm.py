...
#a) 域判别器更新(update_d)：
    def update_d(self, minibatch, opt):
        ...
        all_d1 = minibatch[1].cuda().long()             # 真实类别，这里当作域标签
        all_c1 = minibatch[4].cuda().long()             # 真实潜在域，这里当作类别标签
...
#b) 分类器更新(update)：
    def update(self, data, opt):
        ...
        disc_labels = data[4].cuda().long()                            #从输入的data中获取域标签disc_labels,并将其移动到GPU上,转换为长张量
