...
def plot_features(features, labels, class_labels, title="Feature Visualization", save_path=None):
    ...
    DOMAIN_COLORS = ['#CDD797','#EBB577', '#E09D94']
    ...
    #     loc='best',
    #     fontsize=12,
    #     title="Class & Latent Domain"
    # )
    # legend.get_title().set_fontsize(14)

    legend = plt.legend(
        loc='upper center',
        bbox_to_anchor=(0.5, -0.05),  # x 居中，y 在图外下方
        ncol=3,  # 分两列（4 类很合适）
        fontsize=12,
        # title="Class & Latent Domain",
        frameon=True
...
def plot_domain_alignment_heatmap(true_domains, latent_domains, physical_names, save_path):
    ...
    sns.heatmap(cm_norm, annot=True, fmt=".2f", cmap="YlGnBu",
                ...
                annot_kws={"size": 16, "family": "Arial"}
                )

    # plt.title("Alignment: Physical Domains vs. Mined Latent Domains", fontsize=)
    plt.xlabel("Mined Latent Domains", fontsize=18)
    plt.ylabel("Ground Truth Physical Domains", fontsize=18)
...
def plot_domain_sankey(true_domains, latent_domains, physical_names, save_path):
    ...
    for val, y in y_true.items():
        ...
        plt.text(left_x - 0.05, y, name, ha='right', va='center', fontsize=18, fontweight='bold')
        plt.plot(left_x, y, 'ro', markersize=14)

    for val, y in y_latent.items():
        plt.text(right_x + 0.05, y, f"Latent {val}", ha='left', va='center', fontsize=18, fontweight='bold')
        plt.plot(right_x, y, 'bo', markersize=14)
...
def main():
    ...
    save_dir = os.path.join('results', 'train_output_diversify_domain_alignment', f'{task_name}_{timestamp}')
