...
def main():
    ...
    # 数据加载
    if MODE == 'SINGLE':
        task_name = f"AMINet_Single_{SOURCE_DOMAINS}"
        ...
        task_name = f"AMINet_Multi_{'_'.join(SOURCE_DOMAINS)}"
        if DATASET_TYPE == 'CWRU':
    ...
    save_dir = os.path.join('results', 'aminet_OOD2', f'{task_name}_{timestamp}')
