...
class Config:
    ...
    # ================= 3. 训练与模型参数 =================
    # [新增] 开集故障诊断设置
    # 原始标签: 0:Normal, 1:Inner, 2:Ball, 3:Outer
    # 方案：隐藏 'Ball' (2) 作为未知类.只要你在 config.py 中修改,然后重新运行 train_diversify.py 训练一个新的模型，代码就会自动适配。
    known_classes = [0, 1, 3]  # 训练可见类
    unknown_classes = [2]  # 训练不可见 (OOD)

    # [修改] 网络输出类别数必须等于已知类的数量
    num_classes = len(known_classes)  # 3类 (0,1,2对应Normal, Inner, Outer)
